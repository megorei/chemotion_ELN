...
const ElementsList = ({ overview }) => {
  ...
  useEffect(() => {
    if (!collections || currentCollection?.id == null) return;

    if (currentCollection.shared) {
      if (collections.sharedWithUsers(currentCollection.id) === undefined) {
        collections.getSharedWithUsers(currentCollection.id);
      }
    } else if (collections.isSharedCollection?.(currentCollection.id)) {
      if (collections.mySharesFor(currentCollection.id) === undefined) {
        collections.getMySharesFor(currentCollection.id);
      }
    }
