...
const SequenceBasedMacromoleculeDetails = ({ toggleFullScreen }) => {
  //const sbmmStore = useContext(StoreContext).sbMacromolecules;
  //let sbmm = sbMacromoleculeStore.sb_macromolecule;
  let sbmm = {
    id: 1,
    name: 'Test 1',
    type: 'macromolecule',
    short_label: 'CU6-SBMM-1',
    isNew: true,
    changed: false,
    updated: false,
    can_copy: false,
  ...
  const submitLabel = sbmm.isNew ? 'Create' : 'Save';
  let tabContents = [];
  ...
  visibleTabs.forEach((key, i) => {
    ...
    tabContents.push(
      <Tab eventKey={key} title={title} key={`${key}_${sbmm.id}`} disabled={disabled(i)}>
        {
          !sbmm.isNew &&
          <CommentSection section={`sbmm_${key}`} element={sbmm} />
        }
  ...
  return (
    <Card className={"detail-card"}>
      ...
      <Card.Body>
        ...
        <CommentModal element={sbmm} />
      </Card.Body>
      <Card.Footer>
        <ButtonToolbar className="gap-2">
          <Button variant="primary" onClick={() => DetailActions.close(sbmm)}>
            Close
          </Button>
          <Button variant="warning" disabled={!sbmmIsValid()} onClick={() => handleSubmit()}>
            {submitLabel}
          </Button>
          {downloadAnalysisButton()}
        </ButtonToolbar>
