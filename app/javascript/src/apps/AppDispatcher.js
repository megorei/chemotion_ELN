...
function AppDispatcher() {
  const [role, setRole] = useState(() => UserStore.getState().role);
  console.debug('appDispatcher role = ', role);
  let app = null;

  const handleUserStoreChange = (state) => {
    console.debug('handleUserStoreChange', state);
    setRole(state.role);
  };

  useEffect(() => {
    UserStore.listen(handleUserStoreChange);

    return () => UserStore.unlisten(handleUserStoreChange);
  }, []);
