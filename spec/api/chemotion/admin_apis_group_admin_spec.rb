...
# WP 07 (REQ-ELN-12) regression guard: the admin_* surfaces gate exclusively on
...
describe 'admin API surfaces against group admins' do # rubocop:disable RSpec/DescribeClass -- spans seven API classes
  ...
  before do
    ...
    user.grant_role!(UserRole::GROUP_ADMIN,
                     scope_type: 'group',
                     scope_id: create(:group, admins: [user], users: [user]).id)
  end
