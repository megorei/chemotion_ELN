...
# WP 07 (REQ-ELN-12): POST /messages/new used to trust client-supplied
...
describe Chemotion::MessageAPI do
  ...
  describe 'POST /api/v1/messages/new' do
    ...
    context 'when a group admin holding only a first-class user_roles grant targets the own group' do
      before { user.grant_role!(UserRole::GROUP_ADMIN, scope_type: 'group', scope_id: group_a.id) }

      it 'creates the message (users_admins and user_roles are both honored)' do
        expect { execute_request }.to change(Message, :count).by(1)
      end
    end
